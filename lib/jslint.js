...
t=Object.beget(t);if(type==='(string)'){if(jx.test(value)){warningAt("Script URL.",line,from);}}else if(type==='(identifier)'){if(option.nomen&&(value.charAt(0)==='_'||value.charAt(value.length-1)==='_')){warningAt("Unexpected '_' in '{a}'.",line,from,value);}}
...
if(c<' '){if(c==='\n'||c==='\r'){break;}
warningAt("Control character in string: {a}.",line,character+j,s.slice(0,j));}else if(c==='<'){if(option.safe&&xmode==='xml'){warningAt("ADsafe string violation.",line,character+j);}else if(s.charAt(j+1)==='/'&&((xmode&&xmode!=='CDATA')||option.safe)){warningAt("Expected '<\\/' and instead saw '</'.",line,character);}else if(s.charAt(j+1)==='!'&&((xmode&&xmode!=='CDATA')||option.safe)){warningAt("Unexpected '<!' in a string.",line,character);}}else if(c==='\\'){if(option.safe&&xmode==='xml'){warningAt("ADsafe string violation.",line,character+j);}
...
return it('(number)',t);}
switch(t){case'"':case"'":return string(t);case'//':if(src||(xmode&&!(xmode==='script'||xmode==='CDATA'))){warningAt("Unexpected comment.",line,character);}else if(option.safe&&ax.test(s)){warningAt("ADsafe comment violation.",line,character);}else if(xmode==='script'&&/\<\/script\>/i.test(s)){warningAt("Unexpected <\/script> in comment.",line,character);}
...
prevtoken=token;token=nexttoken;for(;;){nexttoken=lookahead.shift()||lex.token();if(nexttoken.type==='special'){doOption();}else{if(nexttoken.id==='<!['){if(option.safe){error("ADsafe violation.",nexttoken);}
...
function statements(begin){var a=[];if(begin){if(option.safe&&nexttoken.type!=='(string)'){warning('ADsafe violation: Missing "use strict" statement.',nexttoken);}
if(nexttoken.type==='(string)'&&nexttoken.value.slice(0,10)==='use strict'){advance();advance(';');}}
if(option.safe){switch(begin){case'script':if(!adsafe_may){if(nexttoken.value!=='ADSAFE'||peek(0).id!=='.'||(peek(1).value!=='id'&&peek(1).value!=='go')){error('ADsafe violation: Missing ADSAFE.id or ADSAFE.go.',nexttoken);}}
...
x=t.parent;if(option.adsafe){switch(n){case'script':if(stack.length!==1){warning("ADsafe violation: Misplaced script");}
break;case'a':case'applet':case'body':case'embed':case'frame':case'frameset':case'head':case'iframe':case'img':case'object':case'param':warning("ADsafe violation: Disallowed tag: "+n);}}
...
a=a.toLowerCase();if(a==='id'||a==='name'||a==='for'){return a;}
if(n==='script'){if(a==='src'){src=true;return'href';}else if(a==='language'){warning("The 'language' attribute is deprecated.",token);return false;}}else if(n==='style'){if(a==='type'&&option.safe){warning("Don't bother with 'type'.",token);}}
...
t.id=nexttoken.value;if(ids[t.id.toUpperCase()]===true){warning("Duplicate id='{a}'.",nexttoken,nexttoken.value);}else if(option.adsafe){if(adsafe_id){if(stack.length===0){warning("ADsafe violation: A widget can have only one id.",nexttoken);}else if(t.id.slice(0,adsafe_id.length)!==adsafe_id){warning("ADsafe violation: An id must have a '{a}' prefix",nexttoken,adsafe_id);}else if(!/^[A-Z]+_[A-Z0-9_]*$/.test(adsafe_id)){warning("ADSAFE violation: bad id.",t);}}else{adsafe_id=t.id;if(!/^[A-Z]+_$/.test(adsafe_id)){warning("ADSAFE violation: bad id.",t);}}}
ids[t.id]=true;advance();break;case'for':advance('=');if(adsafe_id&&(nexttoken.value.length<=adsafe_id.length||nexttoken.value.slice(0,adsafe_id.length)!==adsafe_id)){warning("ADsafe violation: A for must have a '{a}' prefix",nexttoken,adsafe_id);}
advance();break;case'name':advance('=');if(nexttoken.type!=='(string)'||!nexttoken.value||nexttoken.value===adsafe_id){warning("Bad name attribute.",nexttoken);}
...
if(option.safe&&ux.test(nexttoken.value)){error("ADsafe URL violation.");}
...
break;case'<!':if(option.safe){error("ADsafe HTML violation.");}
for(;;){advance();if(nexttoken.id==='>'){break;}
if(nexttoken.id==='<'||nexttoken.id==='(end)'){error("Missing '{a}'.",token,'>');}}
lex.skip('>');break;case'<!--':if(option.safe){error("ADsafe comment violation.");}
lex.skip('-->');break;case'<%':if(option.safe){error("ADsafe HTML violation.");}
lex.skip('%>');break;case'<?':if(option.safe){error("ADsafe HTML violation.");}
for(;;){advance();if(nexttoken.id==='?>'){break;}
...
countMember(e.value);if(ix.test(e.value)){s=syntax[e.value];if(!s||!s.reserved){warning("['{a}'] is better written in dot notation.",e,e.value);}}}else if(!e||e.type!=='(number)'){if(option.safe){warning('ADsafe subscripting.');}}
...
if(o.adsafe){o.browser=false;o.debug=false;o.eqeqeq=true;o.evil=false;o.forin=false;o.glovar=true;o.nomen=true;o.on=false;o.rhino=false;o.safe=true;o.sidebar=false;o.undef=true;o.widget=false;predefined.Date=false;predefined['eval']=false;predefined.Function=false;predefined.Object=false;predefined.ADSAFE=true;}
