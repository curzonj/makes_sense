# from, http://github.com/collectiveidea/awesomeness/tree/master/recipes/awesomeness/backup.rb
...
namespace :backup do
  ...
  task :create, :roles => :db, :only => {:primary => true} do
    ...
    run_rake "db:backup:create BACKUP_DIR=#{backup_path} SKIP_TABLES=#{skip_tables}"
  end
  ...
  task :mysql, :roles => :db, :only => {:primary => true} do
    run_rake "db:backup:mysql BACKUP_DIR=#{backup_path}"
  end
